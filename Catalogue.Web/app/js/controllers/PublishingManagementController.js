// Generated by IcedCoffeeScript 108.0.11
(function() {
  angular.module('app.controllers').controller('PublishingManagementController', function($scope, $http, $location) {
    var loadList, loadSummary, loadTab1Data, loadTab2Data, loadTab3Data, loadTab4Data, m;
    m = {
      tab: 2,
      publishing: {
        summary: {},
        list: []
      }
    };
    $scope.m = m;
    loadSummary = function() {
      return $http.get('../api/publishing/summary').success(function(result) {
        return m.publishing.summary = result;
      });
    };
    loadTab1Data = function() {
      return $http.get('../api/publishing/publishedsincelastupdated').success(function(result) {
        return m.publishing.list = result;
      });
    };
    loadTab2Data = function() {
      return $http.get('../api/publishing/notpublishedsincelastupdated').success(function(result) {
        return m.publishing.list = result;
      });
    };
    loadTab3Data = function() {
      return $http.get('../api/publishing/publicationneverattempted').success(function(result) {
        return m.publishing.list = result;
      });
    };
    loadTab4Data = function() {
      return $http.get('../api/publishing/pendingsignoff').success(function(result) {
        return m.openData.list = result;
      });
    };
    loadList = function() {
      switch (m.tab) {
        case 1:
          return loadTab1Data();
        case 2:
          return loadTab2Data();
        case 3:
          return loadTab3Data();
        case 4:
          return loadTab4Data();
      }
    };
    $scope.$watch('m.tab', loadList);
    loadSummary();
    return loadTab2Data();
  });

}).call(this);

//# sourceMappingURL=PublishingManagementController.js.map
