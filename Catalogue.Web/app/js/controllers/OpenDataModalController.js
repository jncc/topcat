...
(function() {
  var isAssessedAndUpToDate, isSignedOffAndUpToDate, isUploadedAndUpToDate;
    ...
    refreshAssessmentInfo = function() {
      if ($scope.form.publication.openData.assessment.completed) {
        if ($scope.form.publication.openData.assessment.initialAssessmentWasDoneOnSpreadsheet) {
          ...
          return $scope.assessmentCompletedOnDate = "Completed on: " + moment(new Date($scope.form.publication.openData.assessment.completedOnUtc)).format('DD MMM YYYY h:mm a');
        } else {
          $scope.assessmentCompletedByUser = "Last completed by: " + $scope.form.publication.openData.assessment.completedByUser.displayName;
          return $scope.assessmentCompletedOnDate = "Last completed on: " + moment(new Date($scope.form.publication.openData.assessment.completedOnUtc)).format('DD MMM YYYY h:mm a');
        }
      }
    };
  ...
  isAssessedAndUpToDate = function(record) {
  ...
  isSignedOffAndUpToDate = function(record) {
    if (record.publication === null) {
      return false;
