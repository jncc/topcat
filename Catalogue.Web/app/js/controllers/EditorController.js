...
(function() {
  var addPublishableResource, fakeValidationData, getDataFormatObj, getFormattedDate, getPublishButtonTooltip, getPublishingNotifications, getPublishingText, getResourceUrl, getSecurityText, isFilePath, removePublishableResource, trimDoubleQuotes, updateDataFormatObj;
  ...
  angular.module('app.controllers').controller('EditorController', function($scope, $http, $routeParams, $location, record, $modal, Account) {
    ...
    $scope.isPublishDisabled = function() {
      return !$scope.isSaveHidden() || $scope.form.validation !== 1;
    };
    ...
    $scope.openImagePicker = function() {
      ...
      modal = $modal.open({
        ...
        templateUrl: 'views/partials/imagepicker.html?' + new Date().getTime(),
        size: 'lg',
        scope: $scope,
        resolve: {
  ...
  getPublishButtonTooltip = function(record) {
  ...
  getPublishingNotifications = function(publication, publishingState) {
    var notifications;
    if (publication && publication.target && publishingState) {
      ...
      if (publishingState.assessedAndUpToDate && !publishingState.signedOffAndUpToDate) {
        notifications.push({
          message: 'Awaiting publishing sign off, editing this record will remove it from the approvals list and require re-assessment'
        });
      }
      if ((publication.data && publication.data.lastAttempt && publication.data.lastAttempt.message) || (publication.target.hub && publication.target.hub.lastAttempt && publication.target.hub.lastAttempt.message) || (publication.target.gov && publication.target.gov.lastAttempt && publication.target.gov.lastAttempt.message)) {
        notifications.push({
          message: 'Failure during a previous publishing attempt, please contact Topcat support'
        });
      }
